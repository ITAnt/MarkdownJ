 ...
 * 首页日志适配器
 */
class HomeAdapter(private val mContext: Context, val mList: MutableList<NoteBean>): BaseQuickAdapter<NoteBean, BaseViewHolder>(
    R.layout.item_note, data = mList) {
    ...
    override fun convert(holder: BaseViewHolder, item: NoteBean) {
        ...
        tv_create_time.text = "${TimeUtils.getString(item.createTimeMillis, 0L, TimeConstants.MSEC)}"
        ...
        cv_item.setOnLongClickListener {
            (mContext as? HomeActivity)?.let { activity ->
     ...
     * 重命名
     */
    private fun showRenameDialog(activity: HomeActivity, item: NoteBean) {
        DialogManager.showEditNameDialog(activity, item.noteTitle) {
            // 重命名
            item.noteTitle = it
            (mContext as? HomeActivity)?.run { mLocalNotePresenter.saveNote(item) }
        }
    }
     ...
     * 弹出确认删除对话框
     */
    private fun showDeleteDialog(activity: HomeActivity, item: NoteBean, position: Int) {
